...
export async function PUT(request) {
  try {
    ...
    const containsAdmin = data.some((user) => user.role === "admin");
    if (containsAdmin) {
      return response(
        false,
        400,
        "Action denied. Cannot move an admin user to the trash or restore them."
      );
    }
...
export async function DELETE(request) {
  ...
  try {
    ...
    await UserModel.deleteMany({ _id: { $in: ids } }).session(session);
